...
MsgpackRPCStream.prototype.invoke = function(method, arg, cb, dbg) {
	...
	debug_msg = { 
	    ...
	    "param"  : param,
	    "remote" : this._remote,
	    "dir"    : mpdebug.constants.INCOMING,
	    "seqid"  : seqid,
	    "error"  : null,
	    "reply"  : null
	};
	...
	var new_cb = function (err, res) {
	    debug_msg.dir = mpdebug.constants.OUTGOING;
	    debug_msg.error = err;
	    debug_msg.reply = res;
	    dbg(debug_msg);
	    cb(err, res);
...
function Client (transport, program) {
	...
	if (this._debugger) {
		...
		msg["class"] = mpdebug.constants.CLIENT_CALL;
		this._debugger.mesasge (msg);
	    }
	...
	if (this._debugger) {
	    var debug_hook = function (msg) {
		msg.remote = this.transport.remoteToString()
		msg["class"] = mpdebug.constants.CLIENT_NOTIFY;
		this._debugger.mesasge (msg);
	    }
	    arguments.push(debug_hook);
	}
...
var ServerConnection = function (server, tcpStream) {
	...
	if (this._debugger) {
		...
		"param"  : param,
		"remote" : this._remote,
		"class"  : mpdebug.constants.SERVER,
		"dir"    : mpdebug.constants.INCOMING,
		...
		response.debug_fn = function (err, res) {
		    ...
		    debug_msg.error = err;
		    debug_msg.dir = mpdebug.constants.OUTGOING;
		    this._debugger.message(debug_msg);
		}
	    }
	}
