...
var Server = function (listener) {
	...
	if (self.verbose) {
	    ...
	    s += ("new connection from: " + tcp_stream.remoteAddress + ":" 
		  + tcp_stream.remotePort);
	    console.log (s);
	    
	}
	tcp_stream.on('end', function() { tcp_stream.end(); });
	var rpc_stream = new MsgpackRPCStream(tcp_stream, self.handler);
	if (listener) {
	    listener(rpc_stream);
	}
    });
...
Server.prototype.setVerbose = function (v, n) {
    this.verbose = v;
    this.name = n;
};
