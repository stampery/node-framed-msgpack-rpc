...
var MsgpackRPCStream = function(stream, handler) {
	...
	case REQUEST:
            ...
            self.invokeHandler(method, params, response);
...
MsgpackRPCStream.prototype.invokeHandler = function(method, params, response) 
{
    if (this.handler) {
	this.handler.dispatch (method, params, response);
    }
}
...
var ServerConnection = function (server, tcpStream) {
    ...
    this.addProgram = function (progs) {
	for (var prog in progs) {
	    void methods = progs[prog];
	    for (var method in methods) {
		this.addHandler (method, methods[method], prog);
	    }
	}
    };
    ...
    this.dispatch = function (method, params, response) {
	var handler = this._dispatch[method];
	if (handler) {
	    if (response) { params = params.concat (response); }
...
var Server = function (ConnectionClass) {
    net.Server.call(this);
    var self = this;
    this.verbose = false;
    this.name = "";
    
    this.on('connection', function(tcp_stream) {
	var con = new ConnectionClass (this, tcp_stream);
	con.serve ();
    });
}
...
exports.createServer = function (connectionClass) {
  return new Server (connectionClass);
}
